...
func addDog (_ dog: String, _ mass: inout [String]){
    mass.insert(dog, at: 0)
}

func delDog (_ dog: String, mass: inout [String]) -> String{
    
    guard let position = mass.firstIndex(of: dog) else {return "Нет такого значения"}
    mass.remove(at: position)
    return "Удалено"
}
...
func converter (_ data1: [Int: Double], _ k: Int) -> [Int: Int] {
    
    var data2: [Int: Int] = [:]
    
    for (user, points) in data1.enumerated() {
        
        data2 [user] = Int(points.value / 10) * k
...
let userBalance = converter(userPoints, 1)

print ("Результат: \(userBalance)")
